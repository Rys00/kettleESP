    ...
    def _tryStartThread(func, args=()):
        for x in range(10):
            try:
                ...
                try:
                    start_new_thread(
                        "MWS_THREAD_%s" % _mws_thread_id, func, args
                    )
                    return True
                except:
                    pass
        ...
        if self._handshake(httpResponse):
            ...
            if self._ctrlBuf and self._msgBuf:
                ...
                if threaded:
                    ...
                    return
            print(
                "MicroWebSocket : Out of memory on new WebSocket connection."
            )
        try:
    ...
    def _handshake(self, httpResponse):
        try:
            ...
            if key:
                ...
                httpResponse.WriteSwitchProto(
                    "websocket", {"Sec-WebSocket-Accept": r}
                )
                return True
        except:
            pass
    ...
    def _wsProcess(self, acceptCallback):
        ...
        if self.ClosedCallback:
            try:
                ...
                print(
                    "MicroWebSocket : Error on closed callback (%s)." % str(ex)
                )
